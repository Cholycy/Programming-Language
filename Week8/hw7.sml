...
fun eval_prog (e,env) =
	...
	(case subexp of
	   ...
	   | LineSegment seg => 
			let val (x1, y1, x2, y2) = seg
			in
				LineSegment(x1 + deltax, y1+deltay, x2+deltax, y2+deltay)
			end
	   | Var s => (case List.find (fn (s2,v)=>s=s2) env of
			   NONE => raise BadProgram("var not found: "^s)
			 | SOME (_,v) => eval_prog (Shift(deltax, deltay, v), env))
	   | Intersect(e1, e2) => Shift (deltax, deltay, intersect(eval_prog(e1,env), eval_prog(e2, env)))
	   | _ => e)
...
fun preprocess_prog (e) = 
	case e of 
		...
		then Point(x1, y1)
		    ...
		    if (x1<x2) then LineSegment(seg)
		    else
			if real_close(x1, x2) andalso y1<y2
			then LineSegment(seg)
	  ...
	  | Shift (deltax, deltay, subexp) => Shift (deltax, deltay, preprocess_prog(subexp))
	  | _ => e
